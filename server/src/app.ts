...
/** The configured Express app itself, with no `listen()` call — imported by
 * both src/index.ts (local dev: `app.listen(...)`) and api/index.ts (the
 * Vercel serverless entrypoint, which hands this straight to @vercel/node
 * instead of binding a port). Keeping the two apart means the same route
 * wiring runs in both places without a copy that can drift. */
